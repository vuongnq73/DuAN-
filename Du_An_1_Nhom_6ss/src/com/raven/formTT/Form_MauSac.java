...
public class Form_MauSac extends javax.swing.JFrame {
    ...
    MauSacService mssv= new MauSacService();
      List<Model_MauSac> listxx = new ArrayList<>();
    public Form_MauSac() {
        initComponents();
        model= (DefaultTableModel) tblTT.getModel();
        showData(mssv.selectALll());
        fillThuocTinh(1);
    }
     private static String genMa() {
        int randomNumber = new Random().nextInt(100000);
        String formattedNumber = String.format("%06d", randomNumber);
        String genXX = "MS" + formattedNumber;
        return genXX;
    }
    private void showData(List<Model_MauSac> listXX) {
        int index=1;
            model.setRowCount(0);
            for (Model_MauSac xx : listXX) {
                Object row[]={index++,xx.getMaMauSac(),xx.getTenMauSac()};
                model.addRow(row);
         }
    }
    private void fillThuocTinh(int selected){
            Model_MauSac xx= mssv.selectALll().get(selected);
            txtMaTT.setText(xx.getMaMauSac());
            txtTenTT.setText(xx.getTenMauSac());
        }
     void addTT(){
          ...
          if(count==0){
          Model_MauSac xx= new Model_MauSac(ma, ten);
          Model_MauSac xx1= mssv.selectById(txtMaTT.getText());
          if(xx1!=null){
