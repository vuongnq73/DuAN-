...
public class KhachHangService extends InterfaceCRUD<Model_KhachHang, String>{
    ...
    public void insert(Model_KhachHang entity) {
        sql = "insert into KhachHang(MaKhachHang, HoTen, SoDT, Email, DiaChi, CCCD, TrangThai, CreateBy,UpdateBy) values(?,?,?,?,?,?,?,'NV002','NV002')";
        int check = 0;
        try {
            ...
            ps.setObject(7, entity.isTrangThai());
    ...
    public void update(Model_KhachHang entity) {
        sql = "Update KhachHang set  HoTen =?, SoDT=?, Email=?, DiaChi=?, CCCD=?, TrangThai=?, CreateBy='NV002',UpdateBy='NV002' where MaKhachHang = ? ";
        try {
            con = DBconnect.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public List<Model_KhachHang> selectALll() {
        List<Model_KhachHang> listKH = new ArrayList<>();
        sql = "select MaKhachHang, HoTen, SoDT, Email, DiaChi, CCCD, TrangThai  from KhachHang";
        try {
            con=DBconnect.getConnection();
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
                Model_KhachHang kh = new Model_KhachHang(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getBoolean(7));
                listKH.add(kh);
            }
            return listKH;
        } catch (Exception e) {
    ...
    public Model_KhachHang selectById(String id) {
         ...
         sql = "select MaKhachHang, HoTen, SoDT, Email, DiaChi, CCCD, TrangThai  from KhachHang where MaKhachHang=?";
        try {
             con = DBconnect.getConnection();
            ps = con.prepareStatement(sql);
            ps.setObject(1, id);
    ...
    public List<Model_KhachHang> Find(String MaKhachHang, String HoTen, String SoDT, String Email, String DiaChi, String CCCD, boolean TrangThai) {
        List<Model_KhachHang> listKH = new ArrayList<>();
        sql = "SELECT  MaKhachHang, HoTen, SoDT, Email, DiaChi, CCCD, TrangThai FROM [dbo].[KhachHang] where MaKhachHang like ? or HoTen like ? or SoDT like ? or Email like ? or DiaChi like ? or CCCD like ? or TrangThai like ?";
        try {
            con = DBconnect.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public List<Model_KhachHang> selectByTrangThai(String trangThai) {
        sql = "SELECT MaKhachHang, HoTen, SoDT, Email, DiaChi, CCCD, TrangThai FROM KhachHang WHERE TrangThai=?";
